...
import { loadMatchingDriftReports, loadMatchingRunLogs } from "../replay/drift-loader.js";
...
async function main(): Promise<void> {
  ...
  const totalRunDirs = fs.existsSync(runsDir) ? fs.readdirSync(runsDir).filter((d) => d.startsWith("replay-")).length : 0;
  const matchedRuns = loadMatchingRunLogs(fingerprint, runsDir).length;
  const report = loadMatchingDriftReports(artifact, fingerprint, runsDir);

  console.log(`Drift report: ${artifact.name} v${artifact.version} (${fingerprint})`);
  console.log(`Runs matched: ${matchedRuns} of ${totalRunDirs} replay run(s) under ${runsDir}\n`);
